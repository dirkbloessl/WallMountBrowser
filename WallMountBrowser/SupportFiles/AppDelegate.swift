...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    @objc func applicationDidTimeout(notification: NSNotification) {
        if BulletinDataSource.userDidCompleteSetup {
            let sleepPopupVc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "sleepPopupVc") as! SleepPopupViewController
            let mainWindow = self.window!.rootViewController!
            mainWindow.addChild(sleepPopupVc)
            sleepPopupVc.view.frame = mainWindow.view.frame
            mainWindow.view.addSubview(sleepPopupVc.view)
            sleepPopupVc.didMove(toParent: mainWindow)
        }
    }
}
