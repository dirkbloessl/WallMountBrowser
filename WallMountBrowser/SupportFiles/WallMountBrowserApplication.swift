...
class WallMountBrowserApplication: UIApplication {
    ...
    private var timeoutInSeconds: TimeInterval {
        // 1 minute
        return 1 * UserDefaults.standard.double(forKey: SettingsBundleHelper.SettingsBundleKeys.Timeout)
    }
    ...
    private func resetIdleTimer() {
        ...
        idleTimer = Timer.scheduledTimer(timeInterval: timeoutInSeconds,
                                         target: self,
                                         selector: #selector(WallMountBrowserApplication.timeHasExceeded),
                                         userInfo: nil,
                                         repeats: false
        )
    }
